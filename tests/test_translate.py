...
from aleph_alpha_client.translate import TranslationRequest, TranslationResponse
...
async def test_can_translate_single_request_with_async_client(
    ...
    response = await async_client.translate(request)
...
def test_can_translate_singe_request_with_sync_client(
    sync_client: Client, translation_model_name: str
):
    request = TranslationRequest(
        model=translation_model_name,
        source=LONG_ENGLISH_TEXT,
    ...
    response = sync_client.translate(request)
    assert isinstance(response, TranslationResponse)
    assert response.translation is not None
    assert response.score >= 0.0
    assert response.score <= 1.0
